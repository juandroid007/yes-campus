...
<div class="w-full">
  ...
  >
    <div class="flex flex-col w-full lg:items-center lg:flex-row">
      <div class="hidden mb-4 fill-current stroke-current lg:block w-2/10 lg:mb-0 animate-pulse">
      ...
      <div class="w-full">
        <h1 class="flex mb-6 text-5xl font-bold leading-none md:text-6xl font-title">Yes Talks
          <div class="w-12 ml-2 fill-current stroke-current lg:hidden animate-pulse">
            {#await fetch('/images/mic.svg') then res}
              {#await res.text() then svg}
                {@html svg}
              {/await}
        ...
        <p class="text-3xl font-bold leading-none md:text-4xl font-title">Escucha a nuestra comunidad</p>
      </div>
    </div>
  </div>
